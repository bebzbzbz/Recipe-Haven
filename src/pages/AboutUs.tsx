const AboutUs = () => {
    return (  
        <section>
            <h2 className="text-center">About Us</h2>
            <article className="grid grid-cols-2">
                <div>
                    <p>Hello, we are Amanda and Pedro and we are happy to present our work on Rezeptwelt. While exploring this amazing website, we discovered a gastronomic universe full of delicious recipes, useful tips and culinary inspiration. World of Recipes is a place where experienced chefs and novice cooks can unite in their passion for food.</p>
                    <p>What particularly impressed us was the variety of recipes. From traditional, cosy dishes to more innovative creations, there are options for every taste and every occasion. Each recipe is carefully selected and tested to ensure that the results are always tasty and worth sharing.</p>
                </div>
                <div>
                    <p>In addition to the recipes, World of Recipes also offers useful tips for improving your own cooking skills. From preparation techniques to suggestions for flavour combinations, the website invites you to discover and experiment in the kitchen. It is a cosy and inclusive environment where everyone is encouraged to immerse themselves in the art of cooking and discover new possibilities.</p>
                    <p>In short, World of Recipes is an inspiring gastronomic space that invites us to discover, create and share our passion for cooking. We hope that our presentation has sparked your interest in joining us on this delicious journey into the world of recipes!</p>
                </div>
            </article>
        </section>
    );
}

export default AboutUs;